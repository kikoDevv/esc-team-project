...
async function sortByType() {
    ...
    const onlineArray = challengesArray.filter(challenge => challenge.type === 'online');
    const onsiteArray = challengesArray.filter(challenge => challenge.type === 'onsite');

    if ((includeOnline.checked && includeOnsite.checked) || (!includeOnline.checked && !includeOnsite.checked)) {
...
async function sortByText() {
    ...
    const matchedChallenges = challengesArray.filter(challenge => challenge.title.toLowerCase().includes(input) || challenge.description.toLowerCase().includes(input))
